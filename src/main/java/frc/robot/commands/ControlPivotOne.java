...
public class ControlPivotOne extends CommandBase {
  ...
  public void execute() {
    ...
    if(RobotContainer.pivotOne.get_first_pivot_encoder() >= 53 && speed>0.0){
      speed = 0.0;
    }else if(RobotContainer.pivotOne.get_first_pivot_encoder() <= 1.0 && speed<0.0){
      speed = 0.0;
    }
    RobotContainer.pivotOne.move_pivot_one(speed*0.5);
