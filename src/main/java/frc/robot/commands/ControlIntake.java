...
public class ControlIntake extends CommandBase {
  ...
  public void execute() {
    ...
    if(RobotContainer.intake.get_intake_encoder() <= -5 && speed<0.0){
      speed = 0.0;
    }else if(RobotContainer.intake.get_intake_encoder() >= -2 && speed>0.0){
      speed = 0.0;
    }
    RobotContainer.intake.moveIntake(speed*0.15);
