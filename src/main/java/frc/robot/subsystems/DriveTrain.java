...
public class DriveTrain extends SubsystemBase {
  ...
  public double get_drive_encoder(){
    return driverfrontNEO.getEncoder().getPosition();
  }
  public void stop(){
    drive.stopMotor();
  }
  public void zero_encoder(){
    driverfrontNEO.getEncoder().setPosition(0.0);
  }
  public void brake_mode(){
    driverfrontNEO.setIdleMode(IdleMode.kBrake);
    driverbackNEO.setIdleMode(IdleMode.kBrake);
    passagerfrontNEO.setIdleMode(IdleMode.kBrake);
    passagerbackNEO.setIdleMode(IdleMode.kBrake);
  }
  public void coast_mode(){
    driverfrontNEO.setIdleMode(IdleMode.kCoast);
    driverbackNEO.setIdleMode(IdleMode.kCoast);
    passagerfrontNEO.setIdleMode(IdleMode.kCoast);
    passagerbackNEO.setIdleMode(IdleMode.kCoast);
  }
}